...
class ProfileRepository : IProfileRepository {
    ...
    override suspend fun addToCookedRecipes(
        uid: String,
        recipeId: String,
    ): Result<Unit> {
        return try {
            val doc = Firebase
                ...
                .collection("Profiles")
                .document(uid)
            val cookedList = doc.get().get<List<String>>("cooked")
            if (!cookedList.contains(recipeId)) {
                doc.update(Pair("cooked", FieldValue.arrayUnion(recipeId)))
            }
